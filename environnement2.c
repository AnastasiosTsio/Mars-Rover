...
resultat_deplacement avancer_envt(Environnement *envt) {
  ...
  if ((x < 0) || (x >= largeur(&envt->t)) || (y < 0) ||
    ...
    case LIBRE:
      avancer(&(envt->r));
      envt->etat_abs = transition(envt->etat_abs, A);
      return OK_DEPL;
    case EAU:
      return PLOUF;
    case ROCHER:
...
int mesure_envt(Environnement *envt, int d) {
  ...
  if (d==1){
    envt->etat_abs = transition( envt->etat_abs,M);
  }
...
int resultat_observateur(Environnement *envt){
  return est_final(envt->etat_abs);
}